...
func TestQueryLetter_CaseInsensitiveRow(t *testing.T) {
	// Verify lowercase and uppercase letters at the same position produce the same row
	...
	for i := 0; i < len(lowerLetters); i++ {
		...
		if lowerQuery.MatrixRow != upperQuery.MatrixRow {
			t.Errorf("index %d: lowercase row %d != uppercase row %d", i, lowerQuery.MatrixRow, upperQuery.MatrixRow)
		}
...
func TestMatrix_ExtractPassword_CaseInsensitive(t *testing.T) {
	// Verify that changing case of letters produces the same password
	matrix := newTestMatrix()
	...
	if !bytes.Equal(passLower.Bytes(), passUpper.Bytes()) {
		t.Error("lowercase and uppercase spells produced different passwords (expected same)")
	}
	if !bytes.Equal(passLower.Bytes(), passMixed.Bytes()) {
		t.Error("lowercase and mixed case spells produced different passwords (expected same)")
	}
}
